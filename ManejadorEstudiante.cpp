...
IDictionary *ManejadorEstudiante::getEstNoInscriptos(string numExpediente)
{
    ...
    //se invoca a EstNoInscripto para corroborar si el estudiante est� inscripto a la oferta con numExpediente
...
void ManejadorEstudiante::ModificarEstudiante(string cedula, string nombre, string apellido, string telefono, Date *fechaNacimiento, int creditos, string email, ICollection *aprobacionesAAgregar, IDictionary *asignaturasAEliminar, IDictionary *carrerasAAgregar, IDictionary *carrerasAEliminar)
{
    ...
    if (!carrerasAEliminar->isEmpty())
    {
        IIterator * it = carrerasAEliminar->getIterator();
        while(it->hasCurrent())
        {
            Carrera *carrera;
    ...
    if (!carrerasAAgregar->isEmpty())
    {
        IIterator * it2 = carrerasAAgregar->getIterator();
    ...
    if (!asignaturasAEliminar->isEmpty())
    {
        IIterator * it3 = asignaturasAEliminar->getIterator();
    ...
    if (!aprobacionesAAgregar->isEmpty())
    {
